...
which holds a copy of `notes/` with the hands-on links rewritten to point back at the repo.

```bash
bash docs/publish-notes.sh      # rebuild gh-pages from notes/ and push   (Windows: run in Git Bash or WSL)
```

⚠️ **The repo is private, but the published site is public.** GitHub Pro can build Pages from a private repo, yet
access control for Pages (visible only to collaborators) needs GitHub Enterprise Cloud. Anyone with the URL can read
the notes — which is why only `notes/` is published and `transcript/` never leaves the private repo.
To take the site down: `gh api -X DELETE repos/hemanthreddyllm/mlops-course-notes/pages`.

Prefer automatic deploys on every push? `docs/pages-workflow.yml.example` is ready; the token used here lacked the
`workflow` scope, so it wasn't installed. Enable it with:

```bash
gh auth refresh -s workflow
mkdir -p .github/workflows && git mv docs/pages-workflow.yml.example .github/workflows/pages.yml
